 ...
 * string hrefs (or `<img>`/asset `src`s) for a non-root `base` (BUG-005: every hardcoded
 * `href="/pricing"` etc. site-wide resolved against the origin instead, 404ing in production;
 * BUG-006: the same root cause for `PROVIDER_PHOTO_PLACEHOLDER`'s `<img src>`) — every internal
 * `href`/asset `src` must go through this instead of a hardcoded string, the same
 * `import.meta.env.BASE_URL` pattern `BaseLayout.astro` already uses for font/OG-image URLs.
 * Not for `tel:`/`mailto:`/external URLs, or anything already base-prefixed (e.g.
 * `Astro.url.pathname`).
 */
