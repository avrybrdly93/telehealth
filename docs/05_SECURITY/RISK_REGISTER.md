...
| R-013 | Site reaches launch without HTTP security headers (`X-Content-Type-Options`, `X-Frame-Options`, `Permissions-Policy`, HSTS, CSP `frame-ancestors`) — GitHub Pages cannot deliver any of them and D-012 accepted the gap for Phase 1 | 2 | 3 | 6 | BL-038 is launch-blocking: deliver the full set via CDN/proxy or a host that supports `_headers`, and stand up uptime monitoring, before go-live. Pre-launch exposure is low because the site has no auth, no session cookies, no PHI and no persistence (DATA_BOUNDARIES 1–3), so there is no visitor state for clickjacking or MIME-confusion to reach; that ceases to hold the day a real booking flow ships. CSP + `Referrer-Policy` meta tags already shipped (D-012, 2026-08-03). **Unverified:** whether `github.io` is HSTS-preloaded, which would close the HSTS limb — the sandbox egress proxy blocks `hstspreload.org`; a session with outbound access should check and amend this row | Accepted until launch (D-012, 2026-08-25) |
