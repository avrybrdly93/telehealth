...
- **Last session**: 2026-08-16 (session 70, interactive — practice owner supplied real values directly in conversation, not the scheduled routine). **First session in thirty to change application code.** Set real provider names/phone/pricing and removed CA license numbers site-wide (D-014, new) — see CHANGELOG.md for the full multi-file list and BACKLOG.md BL-012 for what's still outstanding on that item (credentials, approach statements, education/bio copy, photos). Confirmed (no change): the practice does not accept insurance, already matching `BUSINESS_GOALS.md`. **D-009 and D-012 are untouched by this session and remain the real blockers** — still `Proposed`, still gate BL-022 and BL-033, still need the practice owner's decision on hosting. Full local gate re-run and green: lint/format/typecheck clean, `pnpm test` 157/157, `check:readability` 16/0/2 (unchanged), `pnpm build` 21 pages (unchanged). Session 69's verification-only streak (30 sessions, session 40-69) is broken by this entry, not superseded — its findings about D-009/D-012 still stand.
...
| Practice constants | **Set 2026-08-16 (D-014)**: provider names (Ryan Nelson MD, Michael Elhard PMHNP), phone, $200 flat self-pay pricing for both visit types. CA license numbers removed from the site rather than filled (D-014). Still outstanding: provider credentials (e.g. "MD, Board-Certified Psychiatrist"), email, domain, cancellation policy, accepted payment methods (fills remaining `practice.ts` placeholders). |
