...
- **Deploy: green at the current `main` HEAD.** Verified first-hand in session 52: `deploy.yml` run **`31339067675`** (push) at **`693bf8e`** is `success`. **Deploy-run count is a routing artefact, not a health signal — stop treating a single run as a symptom.** The `workflow_run` trigger chains off **`Auto-merge claude branches`**, which fires only `on: push: branches: ['claude/**']`; so a commit landing via a `claude/*` branch yields **two** deploy runs and a commit pushed **straight to `main` yields exactly one**. Sessions 50 and 51 both misread that asymmetry as a trigger regression and "closed" it on a coincidence. Session 38's outage is long cleared; two of its conclusions were wrong and are corrected in the session-39 CHANGELOG entry.
- **The routine's standing "FIRST PRIORITY: `withastro/action@v3` exit-code-1" instruction is stale** and should be edited out of the scheduled prompt. **Twenty-one sessions (32-52)** have confirmed it does not reproduce, and `--frozen-lockfile` has installed cleanly every time, refuting the prompt's lockfile-mismatch hypothesis too. A red `withastro/action@v3` has meant a GitHub 5xx or a scheduling failure every time — **read the job log before suspecting `astro.config.mjs` or `pnpm-lock.yaml`.**
...
**Session 52 did the same as 51: gate re-run, no restatement of the audit, three lines recording only what was new.** Do the same in session 53. Check D-009 (DECISION_LOG line 248) and D-012 (line 434) — if both are still `Proposed` and BACKLOG.md still has no `Ready` row, run the local gate (`pnpm install --frozen-lockfile`, lint, typecheck, test, `pnpm format`, `check:readability`, build), confirm `deploy.yml` is green at the then-current HEAD, and **write at most three lines** saying so. Do not re-derive the human-gated audit — session 33's CHANGELOG entry holds it — and do not invent a task to fill the session. **Do not re-open the deploy-run-count question**: session 52 traced it to the `Auto-merge claude branches` chain (see Build/Deploy bullet above) — one run for a direct push to `main`, two for a `claude/*` landing, both green. Count the runs only if a run *fails*.
...
`ci.yml` can be **dispatched by hand** (BUG-007) — but only when something has actually moved or a push-triggered run hangs; sessions 41-49 all skipped it because deploy was already green at HEAD.

## Weekly Review Findings

_(most recent only; older → CHANGELOG.md)_

- **2026-08-10 (session 52)**: This file is still **68 lines against its own ≤60-line rule — 8 over**, unchanged from sessions 41-51 and stated rather than quietly ignored. Session 52 again edited in place without adding a line. The residual overage is the Blocked / Needs Human Input table plus the two In Progress rows — all live human-gate state, none of it history, and all of it the first thing a returning human needs. **It shrinks when a human resolves D-009 or D-012, not before.** Separately, session 52 closed the deploy-run-count thread that sessions 50 and 51 each spent a bullet on: it is decided by `deploy.yml`'s `workflow_run` trigger chaining off `Auto-merge claude branches` (`on: push: branches: ['claude/**']`), so direct-to-`main` pushes get one run and `claude/*` landings get two. Session 51 declared it a closed one-off on the strength of a single coincidence; the mechanism, not the coincidence, is what closes it. **Standing observation for whoever owns this routine: thirteen sessions (40-52) have now produced no product change, because every remaining item is human-gated.** The schedule is spending three runs a day to re-measure an unchanging gate. Consider pausing the telehealth leg until D-009 is answered.
