...
- **Last session**: 2026-08-16 (session 71, interactive — practice owner supplied "Nelhardson Psychiatric Care" directly in conversation, continuing session 70's same thread). Set `PLACEHOLDER_PRACTICE_NAME` (D-015, new) — the one practice-constant `NEEDS_HUMAN` value D-014 left unresolved. Site-wide: `SiteHeader` logo, `BaseLayout`'s minimal-chrome header, `MedicalBusiness` schema.org `name`. Verified in built HTML, not just source. Full local gate re-run and green: lint/format/typecheck clean, `pnpm test` 157/157 (unchanged), `check:readability` 16/0/2 (unchanged), `pnpm build` 21 pages (unchanged) — no test count change since nothing here is separately test-covered beyond what D-014's session already exercises. **Not verified**: domain/trademark availability for "Nelhardson" — no web access for that from this environment; the practice owner's own responsibility before registering. **D-009 and D-012 remain the real blockers**, untouched by session 70 or 71 — still `Proposed`, still gate BL-022 and BL-033.
- **Build status** — all measured locally in session 69 at `56f6768`, fresh `pnpm install --frozen-lockfile` (Node 22.22.2, pnpm 10.33.0, clean in 6.5s): typecheck **0 errors** (0 warnings, 34 hints); lint, format clean; `pnpm test` **156/156 across 23 files**; `check:readability` **16 passed / 0 failed / 2 skipped**; `pnpm build` **21 pages**. Every figure identical to sessions 40-68, as expected for a session that changed no application code. Playwright and `lhci` were **not** run in session 69; session 39's figures (274 passed / 2 skipped; 21/21 lhci URLs) remain the most recent measurements and are not restated as fresh. Note for future sessions: the format gate is `pnpm format` (it runs `prettier --check`); there is no `format:check` script.
...
| Practice constants | **Set 2026-08-16**: practice name "Nelhardson Psychiatric Care" (D-015), provider names (Ryan Nelson MD, Michael Elhard PMHNP), phone, $200 flat self-pay pricing for both visit types (D-014). CA license numbers removed from the site rather than filled (D-014). Still outstanding: provider credentials (e.g. "MD, Board-Certified Psychiatrist"), email, domain, cancellation policy, accepted payment methods (fills remaining `practice.ts` placeholders). Note: "Nelhardson" has not been checked for domain/trademark availability — that's on the practice owner, not verifiable from this environment. |
...
`ci.yml` can be **dispatched by hand** (BUG-007) — but only when something has actually moved or a push-triggered run hangs; sessions 41-69 all skipped it because deploy was already green at HEAD.
