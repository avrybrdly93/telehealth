...
- **Deploy: green at the current `main` HEAD.** Verified first-hand in session 42: `deploy.yml` run **`31170095532`** at **`39f6ea1`**, all three jobs `success` — build 34s, deploy 13s, smoke **3/3** (homepage 200, sitemap.xml non-empty, `/book` Step 1 renders). Session 38's outage is long cleared; two of its conclusions were wrong and are corrected in the session-39 CHANGELOG entry.
- **The routine's standing "FIRST PRIORITY: `withastro/action@v3` exit-code-1" instruction is stale** and should be edited out of the scheduled prompt. **Ten sessions (32-42)** have confirmed it does not reproduce; in run `31170095532` that step passed **green in 26s**. A red `withastro/action@v3` has meant a GitHub 5xx or a scheduling failure every time — **read the job log before suspecting `astro.config.mjs` or `pnpm-lock.yaml`.**
...
_(none — unchanged since BUG-007 shipped in session 40, re-confirmed in sessions 41 and 42. The project is human-gated: see Blocked / Needs Human Input.)_
...
**Session 42 delivered the verdict session 41 asked for: autonomous sessions have no runway left here.** Do not open session 43 expecting to ship. Check D-009 (DECISION_LOG line 248) and D-012 (line 434) — if both are still `Proposed` and BACKLOG.md still has no `Ready` row, **stop after the local gate and do not write a fourth "nothing claimable" entry**; sessions 41 and 42 already record that state in full, and another restatement adds nothing a human reading this file needs. Confirming the gate is still green (`pnpm install --frozen-lockfile`, lint, typecheck, test, format, `check:readability`, build) is worth the minute; re-deriving the human-gated audit is not — session 33's CHANGELOG entry holds it.
...
`ci.yml` can be **dispatched by hand** (BUG-007) — but only when something has actually moved or a push-triggered run hangs; sessions 41 and 42 both skipped it because deploy was already green at HEAD.

## Weekly Review Findings

_(most recent only; older → CHANGELOG.md)_

- **2026-08-07 (session 42)**: This file is **68 lines against its own ≤60-line rule — 8 over**, unchanged from session 41 and stated rather than quietly ignored. Session 42 rewrote three Snapshot bullets and the Next Session block in place without adding a line, which is the most that editing can do here. The residual overage is the Blocked / Needs Human Input table plus the two In Progress rows — all live human-gate state, none of it history, and all of it the first thing a returning human needs. **It shrinks when a human resolves D-009 or D-012, not before.** Sessions 39 and 41's findings are preserved in their CHANGELOG entries, per this section's "most recent only" rule.
