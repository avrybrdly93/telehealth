...
- **Push triggers are currently unreliable — check, don't assume.** Session 39's close-out push (`9ba82ee`) triggered **neither** `ci.yml` **nor** `deploy.yml`. The deploy was recovered by `workflow_dispatch` (run `31129431506`, all three jobs green), so `main` is deployed and verified; `ci.yml` could **not** be, because it lacks that trigger — which is BUG-007. Every session that pushes must verify via the Actions API that runs actually appeared, and dispatch `deploy.yml` by hand if not. Cause undetermined: consistent with the Actions trouble below, and equally with a regression of BUG-004 (cascaded runs suppressed for actions-authored pushes).
