...
`ci.yml` can be **dispatched by hand** (BUG-007) — but only when something has actually moved or a push-triggered run hangs; sessions 41-57 all skipped it because deploy was already green at HEAD.

## Weekly Review Findings

_(most recent only; older → CHANGELOG.md)_

- **2026-08-11 (session 57)**: This file is **70 lines against its own ≤60-line rule**. Session 57 rewrote lines rather than adding them, so the overage did not grow. The overage is the Blocked / Needs Human Input table plus the two In Progress rows, all live human-gate state, none of it history. **It shrinks when a human resolves D-009 or D-012, not before.** **Standing observation, now eighteen sessions old (40-57): no product change, because every remaining item is human-gated, and the scheduled prompt's "FIRST PRIORITY: `withastro/action@v3` exit-code-1 / check `astro.config` / check the lockfile" has failed to reproduce across twenty-six sessions (32-57) with twenty-six clean `--frozen-lockfile` installs.** Fifteen file-only notes did not reach anyone; sessions 54 and 56 sent notifications on separate dates, which exhausted that channel. **Escalation is closed as an avenue — the remaining lever is the owner's, not this session's.**
