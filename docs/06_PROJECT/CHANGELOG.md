...
- **One new name in the branch check, and it is deliberately recorded as a non-finding.** `claude/festive-meitner-0kobqn` — this session's own harness branch, pre-created by the runner — appears alongside the two known names but is **0 commits ahead of `main`**. `PROJECT_STATUS.md` warned that "a third name appearing here would be new", which is true of an *unmerged* branch and not of this. The check has been sharpened accordingly: report a branch only when it is ahead. A pre-created harness branch will show up under a fresh name every session, and treating each one as a finding would manufacture a new false alarm every run — which is the same failure mode, in a new costume, as the three sessions of shallow-clone lineage finding.
