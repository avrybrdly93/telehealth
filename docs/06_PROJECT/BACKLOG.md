...
|---|---|---|---|---|---|---|
| BL-001 | Scaffold Astro+TS project, repo structure per ARCHITECTURE.md, CI skeleton (lint/typecheck/build), CLAUDE.md at root | M | — | Ready | ARCHITECTURE, TECH_STACK, CODING_STANDARDS, CLAUDE_DEVELOPMENT_PROTOCOL | `pnpm build` green in CI; repo dirs match ARCHITECTURE.md; CLAUDE.md ≤60 lines with 3 absolute rules |
| BL-002 | Design tokens as CSS custom properties + global styles + font self-hosting | S | BL-001 | Ready | DESIGN_TOKENS, DESIGN_SYSTEM, PERFORMANCE_BUDGET | tokens.css matches DESIGN_TOKENS.md exactly; fonts woff2 subsetted ≤120KB; sample page renders both families |
| BL-003 | Content collections + zod schemas (services, providers, conditions, faq, legal) + practice.ts constants with named placeholders | M | BL-001 | Ready | ARCHITECTURE §Content boundary, CODING_STANDARDS §Content | invalid frontmatter fails build; placeholder constants flagged NEEDS_HUMAN |
| BL-004 | Core components batch 1: Button, TextInput, TextArea, Checkbox, Card (+tests, all states) | M | BL-002 | Ready | COMPONENT_LIBRARY, ACCESSIBILITY, ERROR_STATES E-010 | each component: 5 states, keyboard test, axe-clean in test harness |
| BL-005 | SiteHeader, SiteFooter, CrisisResources, SkipLink + base layout | M | BL-004 | Ready | COMPONENT_LIBRARY, INFORMATION_ARCHITECTURE §Navigation, UX-020 | crisis block canonical copy verbatim; mobile menu focus-trapped; axe clean |
...
| ID | Item | Size | Deps | Status | References | Acceptance criteria |
|---|---|---|---|---|---|---|
| BL-010 | Homepage per spec | M | BL-005, BL-003 | Ready | PAGE_SPECIFICATIONS §/, FR-010, COPY_GUIDELINES | FR-010 fold test at 375px passes; GLOBAL-01/02 assertions pass |
| BL-011 | Services index + 2 service pages (content files + template) | M | BL-010 | Ready | PAGE_SPECIFICATIONS §/services, TELEHEALTH_SPECIFICATION | prices from practice.ts; durations stated; NFR-007 demo: page = content file only |
